...
<script>
    ...
    let manpowerRolesData   = @json($manpowerRoles);

    const configs = {
        materials: { countId: 'mat-count', bodyId: 'materials-body' },
        manpower:  { countId: 'mp-count',  bodyId: 'manpower-body'  },
        equipment: { countId: 'eq-count',  bodyId: 'equipment-body' }
    };

    function updateCount(countId, tbody) {
        const rows = tbody.querySelectorAll('tr').length;
        const badge = document.getElementById(countId);
        if (badge) badge.textContent = `${rows} row${rows !== 1 ? 's' : ''}`;
    }
    ...
    function addRow(type) {
        ...
        if (type === 'materials') {
            ...
            productsData.forEach(p => {
                options += `<option value="${p.name}" data-unit="${p.unit}">${p.name}</option>`;
            });
            row.innerHTML = `
                <td>
                    <select name="materials[${idx}][material_name]" class="form-select form-select-sm"
                            onchange="const u=this.options[this.selectedIndex].dataset.unit; if(u) this.closest('tr').querySelector('select[name*=\\[unit\\]]').value=u;">
                        ${options}
                    </select>
                </td>
                <td><input type="number" step="0.001" min="0" name="materials[${idx}][quantity]" class="form-control form-control-sm qty-input" value="0" placeholder="0.000"></td>
                <td>
                    <select name="materials[${idx}][unit]" class="form-select form-select-sm">
                        <option value="">— Unit —</option>
                        <option value="kg">kg</option><option value="m">m</option><option value="m2">m2</option>
                        <option value="m3">m3</option><option value="pcs">pcs</option><option value="liter">liter</option><option value="bag">bag</option>
                    </select>
                </td>
                <td><button type="button" class="btn btn-sm btn-outline-danger row-remove-btn" onclick="removeRow(this,'mat-count')"><i class="fa-solid fa-times"></i></button></td>`;
        } else if (type === 'manpower') {
            let roleOptions = '<option value="">— Select Role —</option>';
            manpowerRolesData.forEach(r => {
                roleOptions += `<option value="${r.name}" data-unit="${r.default_unit}">${r.name}</option>`;
            });
            roleOptions += '<option value="__custom__">+ Type custom role...</option>';

            row.innerHTML = `
                <td>
                    <select name="manpower[${idx}][role]" class="form-select form-select-sm mp-role-select">
                        ${roleOptions}
                    </select>
                ...
                <td><input type="number" step="0.001" min="0" name="manpower[${idx}][quantity]" class="form-control form-control-sm qty-input" value="0" placeholder="0.000"></td>
                <td>
                    <select name="manpower[${idx}][unit]" class="form-select form-select-sm mp-unit-select">
                        <option value="">— Unit —</option>
                        <option value="day">day</option><option value="hr">hr</option><option value="pcs">pcs</option>
                    </select>
                </td>
                <td><button type="button" class="btn btn-sm btn-outline-danger row-remove-btn" onclick="removeRow(this,'mp-count')"><i class="fa-solid fa-times"></i></button></td>`;
        } else if (type === 'equipment') {
            let options = '<option value="">— Select Equipment —</option>';
            equipmentData.forEach(e => {
                options += `<option value="${e.name}" data-unit="${e.unit}">${e.name}</option>`;
            });
            row.innerHTML = `
                <td>
                    <select name="equipment[${idx}][equipment_name]" class="form-select form-select-sm"
                            onchange="const u=this.options[this.selectedIndex].dataset.unit; if(u) this.closest('tr').querySelector('select[name*=\\[unit\\]]').value=u;">
                        ${options}
                    </select>
                </td>
                <td><input type="number" step="0.001" min="0" name="equipment[${idx}][quantity]" class="form-control form-control-sm qty-input" value="0" placeholder="0.000"></td>
                <td>
                    <select name="equipment[${idx}][unit]" class="form-select form-select-sm">
                        <option value="">— Unit —</option>
                        <option value="hr">hr</option><option value="day">day</option><option value="trip">trip</option><option value="pcs">pcs</option>
                    </select>
                </td>
                <td><button type="button" class="btn btn-sm btn-outline-danger row-remove-btn" onclick="removeRow(this,'eq-count')"><i class="fa-solid fa-times"></i></button></td>`;
        }
        tbody.appendChild(row);
        updateCount(cfg.countId, tbody);
    }
    ...
    function saveNewRole() {
        ...
        .then(data => {
            if (data.success) {
                manpowerRolesData.push(data.role);
                manpowerRolesData.sort((a,b) => a.name.localeCompare(b.name));
    ...
    function deleteRole(roleId) {
        ...
        .then(data => {
            if (data.success) {
                manpowerRolesData = manpowerRolesData.filter(r => r.id !== roleId);
                renderModalRolesList();
                syncRoleSelectDropdowns();
            }
        });
    ...
    function syncRoleSelectDropdowns() {
        document.querySelectorAll('.mp-role-select').forEach(select => {
            ...
            manpowerRolesData.forEach(r => {
                options += `<option value="${r.name}" data-unit="${r.default_unit}">${r.name}</option>`;
            });
            options += '<option value="__custom__">+ Type custom role...</option>';
    ...
    const workUnitInput = document.getElementById('workUnitInput');
    function updateUnitLabels() {
        const unit = workUnitInput ? workUnitInput.value.trim() : '';
        const text = unit ? `(per 1 ${unit})` : '';
        document.querySelectorAll('.unit-per-label').forEach(el => el.textContent = text);
    }
    if (workUnitInput) workUnitInput.addEventListener('change', updateUnitLabels);
